...
st.markdown("""
    ...
    .price-card { background: #1c2128; padding: 20px; border-radius: 15px; text-align: center; border: 1px solid #30363d; }
...
def analyze_logic(item):
    try:
        ...
        # 12대 지표 체크
        scores = [
            df['Volume'].iloc[-1] > df['Volume'].iloc[-5:-1].mean()*1.5,
            ma5.iloc[-1] > ma20.iloc[-1],
            curr > df['Open'].iloc[-1],
            curr > df['High'].iloc[-10:-1].max(),
            30 < rsi.iloc[-1] < 70,
            curr <= low_20 * 1.1,
            (close.rolling(20).std()*4/ma20).iloc[-1] < 0.05, # 스퀴즈
            obv.iloc[-1] > obv.iloc[-5],
            (rsi.iloc[-1] > rsi.iloc[-5]) and (curr < close.iloc[-5]), # 다이버전스
            curr > ma20.iloc[-1],
...
def render_compact(res, key):
    ...
    with col1: st.markdown(f"<div class='price-card'><small>진입 권장</small><br><b style='color:#ff4b4b; font-size:18px;'>{res['진입가']}</b></div>", unsafe_allow_html=True)
    with col2: st.markdown(f"<div class='price-card'><small>목표(익절)</small><br><b style='color:#4CAF50; font-size:18px;'>{res['목표가']}</b></div>", unsafe_allow_html=True)
    with col3: st.markdown(f"<div class='price-card'><small>철저 손절</small><br><b style='color:#4b8bff; font-size:18px;'>{res['손절가']}</b></div>", unsafe_allow_html=True)

    names = ["거래량", "골든C", "양봉", "전고돌파", "RSI안전", "바닥지지", "에너지축적", "수급우향", "매집신호", "중심선위", "탄력성", "추세안정"]
    indicators_html = "".join([f"<span class='ind-item' style='background:{'rgba(255,75,75,0.2)' if val else 'transparent'}; border-color:{'#ff4b4b' if val else '#444'}; color:{'#ff4b4b' if val else '#777'}'>{name}</span>" for name, val in zip(names, res['지표'])])
    
    st.markdown(f"<div class='analysis-box'><b>🔍 12대 정밀 분석 (통과: {res['점수']}/12)</b><div class='indicator-grid'>{indicators_html}</div></div>", unsafe_allow_html=True)
...
with t2:
    if st.button("📡 [초고속 딥스캔] SS등급 추출", use_container_width=True):
        with st.spinner("수천 개 종목 분석 중..."):
            try:
                all_l = fdr.StockListing('KRX')
                found = []
                with concurrent.futures.ThreadPoolExecutor(max_workers=40) as ex:
                    futures = [ex.submit(analyze_logic, {'코드': r.Code, '종목명': r.Name}) for r in all_l.itertuples()]
                    for f in concurrent.futures.as_completed(futures):
                        res = f.result()
                        if res: found.append(res)
                st.session_state.rt_results = sorted(found, key=lambda x: (x['등급'], x['점수'], x['수익%']), reverse=True)[:30]
            except: st.error("거래소 서버 지연 중! 다시 시도해줘.")
